...
% 6: Federal Funds Rate     (annualized FFR)
...
num  = [100*log(data.IP) 100*log(data.CPIAUCSL) 100*log(data.CPRINDEX) 100*log(data.TRARR) 100*log(data.BOGNONBR) data.FEDFUNDS];
...
nd        = 1.5e5;                 % number of orthogonal-reduced-form (B,Sigma,Q) draws
...
while record<=nd
    ...
    ppsiy =  -tmpA0(1,1)/tmpA0(6,1);
    ppsip =  -tmpA0(2,1)/tmpA0(6,1);
    
    if (sum(signs{1}*e(:,1)>0))==size(signs{1}*e(:,1),1) && ppsiy>0 && ppsiy<4 && ppsip>0 && ppsip<4
