...
for imodel=mymodels
    ...
    datafilename = 'MFA_04062022';
    sheetuse     = 'US_DFM_SPREAD_Q';
    start_date   = '1976-Q3';
    end_date     = '2019-Q4';    % Last MF/FF observation available for estimation
    
    % Define target periods for density cuts
    target_dates = [
        datenum('2022-Q1',dataformat)
        ];
    ...
    start_plot_var = '1976-Q4';
    end_plot_var   = '2021-Q1';
    ...
    % All dates
    dates_all = datenum((datetime(start_date,'InputFormat',inputformat)):calquarters(1):(datetime(end_plot,'InputFormat',inputformat)))';
    
    % Vector of dates for the full sample    
    dates_full = datenum((datetime(start_date,'InputFormat',inputformat)):calquarters(1):(datetime(end_plot,'InputFormat',inputformat)))';

    % Starting date for plots
    sd = find(datenum((datetime(start_plot,'InputFormat',inputformat)))==dates_full);   
    sd_varp = find(datenum((datetime(start_plot,'InputFormat',inputformat))+calquarters(nlags))==dates_full);   
        
    % End date for plots
    ed_full = find(datenum(end_plot,dataformat)==dates_full);
    ed_varp = find(datenum((datetime(end_date,'InputFormat',inputformat)-calquarters(opt.hh)))==dates_full); 
...
%%
    ...
    [db, db_full,startdb,enddb,tex] = fLoadDataQ(datafilename,sheetuse,start_date,end_date,opt);
 ...
 %%
    ...
    if opt.showfig==1
        ...
        fig0=figure;
        plot(dates_full(sd:ed_full),[MF_full FF_full],'LineWidth',1);
        legend('MF','FF')
        axis tight
        set(gca,'XTick',datenum(dates_full(sd:48:ed_full)))
        datetick('x','yyyy','keepticks')
...
%%
    ...
    if strcmp(opt.paramsUse,'mode')
        ...
        if exist([paramsfolder modelname '.mat'],'file')==0 || opt.force_estimate==1
            ...
            run scriptEstimateMode_FULL_Q.m;
...
%         [quantiles, pdensity] = fPredDensityIteratedFull_RISE(sv,params_in,FF_full,MF_full,GDPG_full,TR_full,opt,simtype,dates_full,db_full);
    ...
    if opt.dir_or_it==1
        % Direct forecast
        % Set MF effect to zero
        params_in.c12=0;
        params_in.c21=0;
        % Set FF effect to zero
%         params_in.b12=0;
%         params_in.b21=0;
        [quantiles_counter,pdensity_counter] = fPredDensityDirectFull(sv,params_in,FF_full,MF_full,GDPGH_full,TR_full,opt,simtype);
        %[quantiles,pdensity] = fPredDensityDirectFull_RISE(sv,params_in,FF_full,MF_full,GDPGH_full,TR_full,opt,simtype,dates_full,db_full);
    elseif opt.dir_or_it==2
        % Iterated forecast      
        % Set MF effect to zero
...
%         params_in.b12=0;
%         params_in.b21=0;
        [quantiles_counter, pdensity_counter] = fPredDensityIteratedFull(sv,params_in,FF_full,MF_full,GDPG_full,TR_full,opt,simtype);
%         [quantiles, pdensity] = fPredDensityIteratedFull_RISE(sv,params_in,FF_full,MF_full,GDPG_full,TR_full,opt,simtype,dates_full,db_full);
    end
...
%         prob = trapz(xi_full(ii,:),pdfi_full(ii,:))
    ...
    if opt.showfig==1
        if opt.selfig(1)
            ...
            l2=plot(dates_full(sd:ed_full), quantiles.mean(sd:ed_full),'-','Color',colors(20,:),'LineWidth', 2,'DisplayName','Mean');
            l3=plot(dates_full(sd:ed_full), quantiles.dYsim_90(sd:ed_full),'-','Color',colors(50,:),'LineWidth', 2,'DisplayName','90th');
            set(gca,'children',flipud(get(gca,'children')))
            l4=plot(dates_full(sd:ed_varp),GDPGH_full_wt(sd:ed_varp),'-.','Color','k','LineWidth', 2,'DisplayName','Realized Value');
            hold off
            ...
            hold on
            if opt.dir_or_it==2
                l4=plot(dates_full(sd:ed_full), 1-quantiles.st_h_mean(sd:ed_full),'-','Color',colors(10,:),'LineWidth', 2.5,'DisplayName','Simulated $s(t+1,t+h|t)$');
                l5=plot(dates_full(sd:ed_full), 1-quantiles_counter.st_h_mean(sd:ed_full),'-.','Color','k','LineWidth', 2.5,'DisplayName','Simulated $s(t+1,t+h|t)$');
            else
                l4=plot(dates_full(sd:ed_full), 1-quantiles.st_t_mean(sd:ed_full),'-','Color',colors(10,:),'LineWidth', 2.5,'DisplayName','Simulated $s(t+1|t)$');
                l5=plot(dates_full(sd:ed_full), 1-quantiles_counter.st_t_mean(sd:ed_full),'-.','Color','k','LineWidth', 2.5,'DisplayName','Simulated $s(t+1|t)$ - Counterfactual');
            end
            hold off
            ylabel('Percent','interpreter','Latex','fontsize',10)
            ...
            hBands=recessionplot;
            set(hBands,'FaceColor','r','FaceAlpha',0.1)
            legend([l4 l5],'Orientation','Vertical','Location','Best','interpreter','Latex');legend boxoff;                    
            ...
            l2=plot(dates_full(sd:ed_full),p12_fitted_ffonly,'k-.','DisplayName','$\hat{p}_{12}$: normal-to-bad (fin only)','LineWidth',3); hold on;
            rr = recessionplot;
            hleg = legend([l1 l2],'Orientation','vertical','Location','North','interpreter','Latex');legend boxoff;
            set(gca,'XTick',datenum(dates_full(sd:numticks:ed_full)))
            datetick('x','yyyy','keepticks')
            set(gca, 'XLim', [dates_full(sd), dates_full(ed_full)])
            ...
            l2=plot(dates_full(sd:ed_full),p21_fitted_ffonly,'k-.','DisplayName','$\hat{p}_{21}$: bad-to-normal (fin only)','LineWidth',3); hold on;
            ylim([0, 1]);
            rr = recessionplot;
            hleg = legend([l1 l2],'Orientation','vertical','Location','North','interpreter','Latex');legend boxoff;
            set(gca,'XTick',datenum(dates_full(sd:numticks:ed_full)))
            ...
            % Set figure appereance
            set(fig01,'PaperOrientation','portrait');
            set(fig01, 'PaperSize', figSize);
            set(fig01, 'PaperUnits', 'inches');
            set(fig01, 'Units','inches');
            set(fig01, 'PaperPositionMode', 'auto');
            set(fig01, 'Position', [figSize(1)/5 figSize(2)/5 figSize(1) figSize(2)]);
            tightfig;
...
%         psr = ps((recs(ii)-5):(rece(ii)+5));
    ...
    if opt.showfig==1
        ...
        %         text(xlimits(1)*0.865,0.8*ylimits(2),['Prob bad regime = ' num2str((1-quantiles.st_t_mean(opt.date_index(ii)))*100) '\%'],'FontSize',13,'Interpreter','Latex');
                ...
                if opt.dir_or_it==2
                    plot(zeros(1,length(xi_bad(1,:))),zeros(1,length(pdfi_bad(1,:))),'*','DisplayName',['P(Bad Regime $(t+1,t+h|t)$) = ' num2str(round((1-quantiles.st_h_mean(opt.date_index(ii)))*100,1)) '\%']);
                else
                    plot(zeros(1,length(xi_bad(1,:))),zeros(1,length(pdfi_bad(1,:))),'*','DisplayName',['P(Bad Regime $(t+1,t+h|t)$) = ' num2str(round((1-quantiles.st_t_mean(opt.date_index(ii)))*100,1)) '\%']);
                end
                vline(0,'k--');
    ...
    if opt.showfig && opt.saveit
        if opt.selfig(1)
            ...
            print('-dpdf',fig01,[figfolder 'TS_Prob_' modelname ],'-bestfit');
        end
        if opt.selfig(2)
