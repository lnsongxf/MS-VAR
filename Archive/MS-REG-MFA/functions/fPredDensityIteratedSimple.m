...
%
%------------------------------------------------------------------
function [out, pden_mat, badind] = fPredDensityIteratedSimple(sv,param_in,FF,MF,trend_fit,opt,temp,modelspec)
...
for dd=1:opt.nParamDraws
    ...
    [param,bad] = scriptParams_SIMPLE(param_in,dd,modelspec);
    badind(dd,1) = bad;
    ...
    parfor uu=1:opt.nDraws
        ...
        for tt=1:tperiods
            ...
            [y_temp,y_temp1,y_temp2] = simulateSimple(st_sim,f_t,m_t,param,opt,bad);    
            ...
            elseif bad==1
                %disp('bad=1')
                y_mat_fut_ex_temp_good(dd,uu,tt) = mean(y_temp2(1:opt.hh,1)) + trend_fit(tt);
                y_mat_fut_ex_temp_bad(dd,uu,tt) = mean(y_temp1(1:opt.hh,1)) + trend_fit(tt);                
            end
