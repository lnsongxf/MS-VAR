...
%==================================================================================
function [db, db_full,startdb,enddb, tex]= fLoadDataQ(datafilename,sheetuse,start_date,end_date,hh)
...
rawdata = readtable(['data/' datafilename '.xlsx'],'Sheet',sheetuse);
...
d = [GDPGH(1:(e-s+1)-hh,:) FF(1:(e-s+1)-hh,:) MF(1:(e-s+1)-hh,:) GDP_GROWTH(1:(e-s+1)-hh,:) y_dfm_trend(1:(e-s+1)-hh,:) yh_trend(1:(e-s+1)-hh,:)]; % growth rates
...
% ***Truncate estimation samples 12-months prior to enddate***
startdb = [num2str(year(datetime(start_date,'InputFormat','yyyy-MMM'))) 'Q' num2str(quarter(datetime(start_date,'InputFormat','yyyy-MMM')))];
enddb   = [num2str(year(datetime(Dates(e-hh),'InputFormat','yyyy-MMM'))) 'Q' num2str(quarter(datetime(Dates(e-hh),'InputFormat','yyyy-MMM')))];
