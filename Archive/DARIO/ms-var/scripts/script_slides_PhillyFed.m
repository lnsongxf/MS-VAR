...
load(['QR_results/quantiles_QR_new']);
...
%     load('QR_results/results___Country=US___GDP=GDP___SpecWith=ff_mf___Scenario=Baseline___Lags=0___Detrended=given___Sample=1973-Jan_to_2020-May.mat')
    load('QR_results/results___Country=US___GDP=GDP___SpecWith=ff_mf___Scenario=November30___Lags=0___Detrended=given___Sample=1973-Jan_to_2020-Oct.mat')
end

ed_full = size(YQ_dist_fut,1);
...
subplot(211)
l1=plot(dates_full(sd:ed_full),p12_fitted(sd:ed_full),'color',colors2(1,:),'DisplayName','$\hat{p}_{12}$: normal-to-bad','LineWidth',3); hold on;
axis tight; ylim([0, 1]);
rr=recessionplot;
hleg = legend([l1],'Orientation','vertical','Location','North','interpreter','Latex');legend boxoff;
ax=gca;
% ax.XTick = datenum(dates(sd:numticks2:ed_full));
...
subplot(211)
l1=plot(dates_full(sd:ed_full),p21_fitted(sd:ed_full),'color',colors2(16,:),'DisplayName','$\hat{p}_{21}$: bad-to-normal','LineWidth',3); hold on;
axis tight; ylim([0, 1]);
rr=recessionplot;
hleg = legend([l1],'Orientation','vertical','Location','North','interpreter','Latex');legend boxoff;
ax=gca;
...
%--------------------------------------------------------------------------
fig5=figure; clf;
l2=plot(dates_full(sd:ed_full),p_reg2_sim(sd:ed_full),'-','LineWidth',2,'DisplayName','Pseudo Real-Time Simulation of $P(s_t=2)$'); hold on
ylim([0 1.0]);
rr=recessionplot;
