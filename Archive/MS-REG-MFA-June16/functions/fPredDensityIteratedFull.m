...
%
%------------------------------------------------------------------
function [out, pden_mat] = fPredDensityIteratedFull(sv,params_in,YY,trend_fit,opt,temp)
...
%     for uu=1:nDraws
        ...
        for tt=nlags:tperiods            
            ...
            for vv=1:nvars
                if vv==idxg
                    y_mat_fut_ex_temp(dd,uu,vv,tt) = mean(y_temp(vv,:),2)+trend_fit(tt);
                    y_mat_fut_ex_temp_good(dd,uu,vv,tt) = mean(y_temp1(vv,:),2)+trend_fit(tt);
                    y_mat_fut_ex_temp_bad(dd,uu,vv,tt)  = mean(y_temp2(vv,:),2)+trend_fit(tt);
                else
                    y_mat_fut_ex_temp(dd,uu,vv,tt) = mean(y_temp(vv,:),2);
                    y_mat_fut_ex_temp_good(dd,uu,vv,tt) = mean(y_temp1(vv,:),2);
                    y_mat_fut_ex_temp_bad(dd,uu,vv,tt)  =  mean(y_temp2(vv,:),2);
                end
            end
