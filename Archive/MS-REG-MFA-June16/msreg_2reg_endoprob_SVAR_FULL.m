...
% June 10, 2022 CHANGE: Model can accommodate >3 variables
...
opt.selfig         = [1 1 1 zeros(1,8)];  % 1 = select figures
...
for ifor = myfor
    ...
    for ivar=myvars
        ...
        if ivar==3
            varlist={'FF','MF',opt.gname}';
        elseif ivar==4
            varlist={'FF','MF',opt.gname,'FEDFUNDS'}';
        elseif ivar==5
            varlist={'FF','MF',opt.gname,'PCEPILFE_CCA','FEDFUNDS'}';
        end
        opt.varlist = varlist;
        ...
        for imodel=mymodels
            ...
            for jj=1:length(varlist)
                eval(['YY(:,jj) = db_full.' varlist{jj} '.data;']);
            end
            % Collect trend
            TR_full       = db_full.TRENDH.data;
            
            % Complete trend with last observation
            for jj=1:length(TR_full)
                if TR_full(jj)==0
                    TR_full(jj) = TR_full(jj-1);
                end
            end
            
            % To obtain GDP(t+1:t+h) we need to offset GDPGH (i.e., GDP(t:t+h-1)) by one period
            % Used when showing realized growth outlook in charts
            GDPGH_full_wt    = [db_full.GDPGH.data(2:end)+db_full.TRENDH.data(2:end); 0];
            GDPGH_full_wt((end-opt.hh+1):end) = NaN;
            ...
            if exist([paramsfolder modelname '_results.mat'],'file')==0 || opt.force_simulate==1
                ...
                if opt.dir_or_it==1
                    % Direct forecast
                    [quantiles,pdensity] = fPredDensityDirectFull(sv,params_in,YY,TR_full,opt,simtype);
                elseif opt.dir_or_it==2
                    % Iterated forecast
                    [quantiles, pdensity] = fPredDensityIteratedFull(sv,params_in,YY,TR_full,opt,simtype);
                end
                ...
                % Now focus on GDP
                if opt.dir_or_it ==1
                    idxg = find(ismember(varlist, 'GDPGH'));
                elseif opt.dir_or_it ==2
                    idxg = find(ismember(varlist, 'GDPG'));
                end
                id = idxg;
                ...
                for ii=1:ed_eval
                    [~,xi_ps]  = min(abs(xi_full(ii,id,:)-GDPGH_full_wt(ii,:)));
                ...
                % Do not use last hh observations since no data for GDPGH               
                for ii=1:ed_eval
                    [~,xi_pits]  = min(abs(cxi_full(ii,id,:)-GDPGH_full_wt(ii,:)));
            ...
            % Realized values of non-GDP variables
            if opt.dir_or_it ==1           
             % For direct model, gather data vector of next period Y for 
               YYrealized = [YY(2:end,:);NaN(1,opt.nvars)];
            else
             % For iterated model, gather data vector average over next hh months
                YYrealized = filter(ones(1,opt.hh)/opt.hh, 1, YY);
                YYrealized(1:(opt.hh-1),:) = NaN;
                YYrealized = YYrealized((opt.hh+1):end,:);
                YYrealized = [YYrealized; NaN(opt.hh,opt.nvars)];               
            end
            ...
            figureTitleTag = [strrep(spectag,'_',' ') ', ' opt.paramsUse ', ' strrep(opt.sheetuse,'_',' ') ', h=' num2str(opt.hh) ', ' startdb ':' enddb];
            ...
            if opt.showfig==1
                if opt.selfig(1)
                    for nn=1:opt.nvars
                        ...
                        if nn==idxg
                            l3=plot(dates_full(sd:ed_full),GDPGH_full_wt(sd:ed_full),'-.','Color','k','LineWidth', 2,'DisplayName','Realized Value');
                        else
                            l3=plot(dates_full(sd:ed_full),YYrealized(sd:ed_full,nn),'-.','Color','k','LineWidth', 2,'DisplayName','Realized Value');
                        end
                        set(gca,'children',flipud(get(gca,'children')))
                        hold off
%                         ylabel('Percent','interpreter','Latex','fontsize',10)
                        if nn==idxg
                            ylabel('Percent','fontsize',10,'Interpreter','Latex')
                        else
                            ylabel('Units of Measure','fontsize',10,'Interpreter','Latex')
                        end
                        if opt.dir_or_it==1
                            mytitle1 = 'Direct';
                            if nn==idx
                                mytitle2 = ['Average ' tex.(opt.varlist{nn}) ' over Next ' num2str(opt.hh) ' Months'];
                            else
                                mytitle2 = [tex.(opt.varlist{nn}) ' Next Month'];
                        ...
                        title({['Quantiles ' mytitle1 ' : '  figureTitleTag ] ; mytitle2},'Interpreter','Latex','FontSize',16);
                        axis tight
                        ...
                        tightfig;
                        if opt.saveit
                            print('-dpdf',fig1,[figfolder 'Quantiles_' varlist{nn} '_' modelname ],'-bestfit');
                        end
                    end
                end
            end
            ...
            if opt.const==1
                ...
                elseif opt.transprob==1
                    if opt.normal==1
                        tp_nb=1./(1+exp(a12-b12*YY(:,idxf)-c12*YY(:,idxm)));
                        tp_bn=1./(1+exp(a21-b21*YY(:,idxf)-c21*YY(:,idxm)));
                    elseif opt.normal==0
            ...
            elseif opt.const==0
                ...
                elseif opt.transprob==1
                    if opt.normal==1
                            tp_nb=1./(1+exp(-b12*YY(:,idxf)-c12*YY(:,idxm)));
                            tp_bn=1./(1+exp(-b21*YY(:,idxf)-c21*YY(:,idxm)));
                    elseif opt.normal==0
                            tp_nb=1./(1+exp(-b12*YY(:,idxf)+c12*YY(:,idxm)));
                            tp_bn=1./(1+exp(+b21*YY(:,idxf)-c21*YY(:,idxm)));
                    end
            ...
            if opt.showfig==1
                if opt.selfig(2)
                        ...
                        title('Probability of Switching from Normal to Bad Regime')
                        axis tight
                        set(gca,'XTick',datenum(dates_full(sd:numticks:ed_full)))
                        datetick('x','yyyy','keepticks')
                        set(gca, 'XLim', [dates_full(sd), dates_full(ed_full)])
                        ...
                        title('Probability of Switching from Bad to Normal Regime')
                        axis tight
                        set(gca,'XTick',datenum(dates_full(sd:numticks:ed_full)))
                        datetick('x','yyyy','keepticks')
                        set(gca, 'XLim', [dates_full(sd), dates_full(ed_full)])
...
%                             vline(GDPGH_full_wt(opt.date_index(ii)),'r:','Realized Value');
                            ...
                            xlabel(['Average ' tex.(opt.varlist{nn}) ' over Next ' num2str(opt.hh) ' Months'],'fontsize',10,'Interpreter','Latex')
                            axis tight
                            if ii==1
                                ylimits = ylim; 
                                xlimits = xlim;
                            ...
                            tightfig;
                            if opt.saveit
                                print('-dpdf',fig3,[figfolder 'Densities_' varlist{nn} '_' datestr(dates_full(opt.date_index(ii)),dataformat) '_' modelname ],'-bestfit');
                            end
                        end
                    end
                end
            ...
            if opt.showfig==1
                if opt.selfig(7)
                    ...
                    set(fig6, 'Units','inches');
                    set(fig6, 'PaperPositionMode', 'auto');
                    set(fig6, 'Position', [figSize7(1)/5 figSize7(2)/5 figSize7(1) figSize7(2)]);
                    tightfig;
                    if opt.saveit
