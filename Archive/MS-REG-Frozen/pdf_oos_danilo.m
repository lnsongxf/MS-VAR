...
specs     = [1]; % select which model(s) want to run
...
%                             plot(ax1,ResMatch.YY,PST,linS{ss},'LineWidth',2,'DisplayName',plname,'Color',color);
                            plot(ax1,ResMatch.YY,PST,linS{ss},'LineWidth',2,'DisplayName','Quantile Regression density','Color',color);
                            [~,i10] = min(abs(CDF-0.1));
                            ...
                            if ss==length(specs)
                                ymax = max(ym)*1.05;
                                plot(ax1,[yh(jt+ws+hh) yh(jt+ws+hh)],[0 ymax],'r','DisplayName',['Realization = ' num2str(round(yh(jt+ws+hh),1)) '\%']);
                                ...
                                tightfig;
                                if saveit==1
                                    savefig(fig,[folder 'Danilo_PDF_OOS_' '___Period=' char(period_pdf) '___Country=' country_filename '___GDP=' gdp_filename '___Scenario=' scen_name '___Lags=' num2str(p) '___Detrended=' dt '___Sample=' startdate '_to_' enddate]);
                                    print('-dpdf',fig,[folder 'Danilo_PDF_OOS_' '___Period=' char(period_pdf) '___Country=' country_filename '___GDP=' gdp_filename  '___Scenario=' scen_name '___Lags=' num2str(p) '___Detrended=' dt '___Sample=' startdate '_to_' enddate],'-bestfit');
                                    saveas(fig,sprintf('%s.png',[folder 'Danilo_PDF_OOS_' '___Period=' char(period_pdf) '___Country=' country_filename '___GDP=' gdp_filename '___SpecWith=' spec_name '___Scenario=' scen_name '___Lags=' num2str(p) '___Detrended=' dt '___Sample=' startdate '_to_' enddate ]));
                                end
                            end
                        end
                    end
