...
if compare ==2
    if exist([sim_folder 'results_iterated.mat'],'file')==2
        ...
        st_dgp_t = NaN(size(y_mat,1),size(y_mat,2)); % This stores all realizations for the ITERATED
        ...
        for jj=1:size(y_mat,2)
            ...
            for uu=1:num_rep
                ...
                if jj ==1
                    st_temp_par = Res_iterated.st_mat(dd,jj);
                else
                    st_temp_par = Res_iterated.st_mat(dd,jj-1);
                end
                ...
                p12 = 1/(1+exp(dgp.a12-dgp.b12*(f_temp(1))+dgp.c12*(m_temp(1))));
                p21 = 1/(1+exp(dgp.a21+dgp.b21*(f_temp(1))-dgp.c21*(m_temp(1))));
                p11 = 1 - p12; % probability of remaining in normal
                p22 = 1 - p21; % probability of remaining in bad
        ...
        st_dgp_t_50 =        mean(st_dgp_t)'; % This is in t+12
        dY_25_fut_ex_dgp = prctile(y_mat_fut_ex_dgp,25)'; dY_75_fut_ex_dgp = prctile(y_mat_fut_ex_dgp,75)';
        dY_10_fut_ex_dgp = prctile(y_mat_fut_ex_dgp,10)'; dY_90_fut_ex_dgp = prctile(y_mat_fut_ex_dgp,90)';
        ...
        if exist([sim_folder 'results_direct.mat'],'file')==2
            ...
            for jj=13:size(y_mat,2)
                ...
                f_temp = ff_temp_par1(jj-11:jj);
                m_temp = mf_temp_par1(jj-11:jj);
                st_temp_par = st_temp_par1(jj-12);
                parfor dd=1:nParamDraws
                    ...
                    for uu=1:nDraws
                        ...
                        for ee=1:12 % Construct states for every period from t-12 to t, then check the state at t and construct the direct GDP (t+1 to t+12)
            ...
            hold on
            l1=plot(size_in:size_ex, dY_10_fut_ex(size_in:size_ex),'--k','Color',colors(5,:),'LineWidth', 1.5,'DisplayName','Iterated draw 10th');
            l4=plot(size_in:size_ex, dY_90_fut_ex(size_in:size_ex),'--k','Color',colors(55,:),'LineWidth', 1.5,'DisplayName','Iterated draw 90th');
            l11=plot(size_in:size_ex, dY_10_fut_ex2(size_in:size_ex),'-k','Color',colors(5,:),'LineWidth', 1.5,'DisplayName','Direct draw 10th');
            l44=plot(size_in:size_ex, dY_90_fut_ex2(size_in:size_ex),'-k','Color',colors(55,:),'LineWidth', 1.5,'DisplayName','Direct draw 90th');
            l111=plot(size_in:size_ex, dY_10_fut_ex_dgp(size_in:size_ex),'-.k','Color',colors(5,:),'LineWidth', 1.5,'DisplayName','DGP draw 10th');
            l444=plot(size_in:size_ex, dY_90_fut_ex_dgp(size_in:size_ex),'-.k','Color',colors(55,:),'LineWidth', 1.5,'DisplayName','DGP draw 90th');
            %             l2=plot(13:size_ex, Res_direct.y_mat(example_draw,14:size_ex+1),'-k','LineWidth', 1.5,'DisplayName','Data');
            %             legend([l1 l4 l11 l44 l111 l444 l2],'Orientation','Vertical','Location','Best','interpreter','Latex');legend boxoff;
            ...
            fig=figure('Units','normalized','Position', [0,0,1.2,0.9]); clf;
            hold on
            l1=plot(size_in:size_ex, st_it_t_50(size_in:size_ex),'--k','Color',colors(5,:),'LineWidth', 1.5,'DisplayName','Iterated draw state (mean)');
            l11=plot(size_in:size_ex, st_dir_50(size_in:size_ex),'-k','Color',colors(5,:),'LineWidth', 1.5,'DisplayName','Direct draw state (mean)');
...
%             l111=plot(13:size_ex, st_dgp_t(example_draw,13:size_ex),'-k','LineWidth', 1.5,'DisplayName','DGP draw state (mean)');
            l111=plot(size_in:size_ex, st_dgp_t_50(size_in:size_ex),'-k','LineWidth', 1.5,'DisplayName','DGP draw state (mean)');
            %                         l2=plot(13:size_ex, Res_direct.y_mat(example_draw,14:size_ex+1),'-k','LineWidth', 1.5,'DisplayName','Data');
            %             legend([l1 l4 l11 l44 l111 l444 l2],'Orientation','Vertical','Location','Best','interpreter','Latex');legend boxoff;
