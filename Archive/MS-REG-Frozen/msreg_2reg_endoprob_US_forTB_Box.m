...
datafilename = 'MacroRisk_04122021.xlsx';
...
start_plot    = '1973-Feb';
end_plot      = '2019-Oct';
end_plot_full = '2021-Apr';

% Vector of dates for the full sample
dates_full = datenum((datetime(start_date,'InputFormat',inputformat))+calmonths(nlags):calmonths(1):(datetime(end_plot_full,'InputFormat',inputformat)))';
...
fit1 = Fits.GDPGH.data(:,1)+db.TRENDH.data(nlags+1:ed+1);
fit2 = Fits.GDPGH.data(:,2)+db.TRENDH.data(nlags+1:ed+1);
...
MF_full = db_full.MF.data(nlags+1:end);
TR_full = db_full.TRENDH.data(nlags+1:end);
fprintf('\n Simulating quantiles for the full sample, wait.... \n');
...
l1=plot(dates_full, dY_sim_10_full,'Color',colors(5,:),'LineWidth', 1.5,'DisplayName','MS 10th');
l2=plot(dates_full, dY_sim_25_full,'Color',colors(15,:),'LineWidth', 2,'DisplayName','MS 25th');
l3=plot(dates_full, dY_sim_75_full,'Color',colors(45,:),'LineWidth', 2,'DisplayName','MS 75th');
l4=plot(dates_full, dY_sim_90_full,'Color',colors(55,:),'LineWidth', 1.5,'DisplayName','MS 90th');
rr = recessionplot;
...
ylabel('Percent','interpreter','Latex','fontsize',10)
axis tight
setfig(fig,dates_full,sd,ed_full,numticks,FontSize,figSize,'Quantiles MS Model');
...
fig=figure; clf;
hold on
l1=plot(dates_full(sd:ed), dY_sim_25_full(sd:ed),'Color',colors(15,:),'LineWidth', 2,'DisplayName','MS 25th');
l2=plot(dates_full(sd:ed), dY_sim_75_full(sd:ed),'Color',colors(55,:),'LineWidth', 2,'DisplayName','MS 75th');
l3=plot(dates_full(sd:ed), dY_sim1_mean_full(sd:ed),'-.','Color',colors(45,:),'LineWidth', 1.5,'DisplayName','$E(Good Regime)$');
l4=plot(dates_full(sd:ed), dY_sim2_mean_full(sd:ed),'--','Color',colors(12,:),'LineWidth', 1.5,'DisplayName','$E(Bad Regime)$');
% Format plot
hleg = legend([l1 l2 l3 l4],'Orientation','Vertical','Location','South','interpreter','Latex');legend boxoff;
set(gca,'children',flipud(get(gca,'children')))
ylabel('Percent','interpreter','Latex','fontsize',10)
setfig(fig,dates_full,sd,ed_full,numticks,FontSize,figSize,'Expected Value in Each Regime');
...
for tt=1:length(prob_reg2_new_full)
    ...
    pdY = fitdist(dY_sim_full(tt,:)','Kernel','Kernel','epanechnikov');

    percentile_reg1(tt,1) = cdf(pdY,dY_sim1_mean_full(tt,1));
    percentile_reg2(tt,1) = cdf(pdY,dY_sim2_mean_full(tt,1));
end
...
data_use.prob_reg2_new = prob_reg2_new_full;
...
outApr2021 = fGetDensityMS('2021-Apr',data_use,a2tilde_to_a,results,pnames,nDraws,nParamDraws);
...
[pdf_apr21,xi_apr21]=ksdensity(outApr2021.y_erg_bar(1,:));
[cdf_apr21,xic_apr21]=ksdensity(outApr2021.y_erg_bar(1,:),'function','cdf');
...
[pdf_oct_1,xi_oct_1]=ksdensity(outApr2021.y_erg_bar_1(1,:));
[pdf_oct_2,xi_oct_2]=ksdensity(outApr2021.y_erg_bar_2(1,:));
[pdf_mar_1,xi_mar_1]=ksdensity(outMar2020.y_erg_bar_1(1,:));
[pdf_mar_2,xi_mar_2]=ksdensity(outMar2020.y_erg_bar_2(1,:));

%% Figure: Good vs Bad Regimes
...
l2=plot(xi_apr21,pdf_apr21,'Color',colors(60,:),'LineWidth', 3,'DisplayName',['April-2021 ']);
