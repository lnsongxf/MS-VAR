...
eta_1_bad = RESID_S_bad;       eta_1_norm = RESID_S_norm;       % These are the residuals of GDP = a1*FF(-1) + b1(MF(-1)
eta_2_bad = RESID_FF(p_bad,:); eta_2_norm = RESID_FF(p_norm,:); % These are the residuals of FF = a2*FF(-1)
eta_3_bad = RESID_MF(p_bad,:); eta_3_norm = RESID_MF(p_norm,:); % These are the residuals of MF = a3*FF(-1)

% FF depends on MF, so let's clean it
A_res_bad_eta_2 = (eta_3_bad'*eta_3_bad)\(eta_3_bad'*eta_2_bad);
A_res_norm_eta_2 = (eta_3_norm'*eta_3_norm)\(eta_3_norm'*eta_2_norm);
eta_2_bad_hat = (eta_2_bad - eta_3_bad*A_res_bad_eta_2);
eta_2_norm_hat = (eta_2_norm - eta_3_norm*A_res_norm_eta_2);

% Now, regress eta_1 on eta_2 and eta_3; the residual of this should be informative about c_11* e_1
% X_res_bad = [eta_2_bad eta_3_bad];
X_res_bad = [eta_2_bad_hat eta_3_bad];
A_res_bad = (X_res_bad'*X_res_bad)\(X_res_bad'*eta_1_bad);
% X_res_norm = [eta_2_norm eta_3_norm];
X_res_norm = [eta_2_norm_hat eta_3_norm];
A_res_norm = (X_res_norm'*X_res_norm)\(X_res_norm'*eta_1_norm);
...
c11_2 = std(eta_1_bad - X_res_bad*A_res_bad); c11_1 = std(eta_1_norm-X_res_norm*A_res_norm);
