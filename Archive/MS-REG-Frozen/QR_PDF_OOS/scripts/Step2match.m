function Res = Step2match(YQ,YQunc,QQ,YY,delta,Qmatch)
...
qqTarg = YQunc;
[lcn,scn,shn,dfn] = QuantilesInterpolation(qqTarg,QQ,Qmatch);
...
CST            = NaN(size(YQ,1),length(YY));
ProbST_lt0  = NaN(size(YQ,1),1);
QST_store     = NaN(size(YQ));
STpar_store   = NaN(size(YQ,1),4);
...
parfor jt = 1:size(YQ,1)
    ...
    if jt==size(YQ,1)-1 || jt==size(YQ,1)
        Qmatch2 = [0.15 0.25 0.65 0.99];
        QQ2 = QQ;
    else 
        Qmatch2 = Qmatch;
        QQ2 = QQ;
    ...
    if ~any(isnan(qqTarg))
        ...
        %%%       initial conditions
        %if jt<=5 || mod(jt,4)==0
            [lc,sc,sh,df]  = QuantilesInterpolation(qqTarg,QQ2,Qmatch2);
        %else
        ...
        CST(jt,:)   = pskt(YY,lc,sc,sh,df);
        ProbST_lt0(jt,:) = pskt(0,lc,sc,sh,df);
        QST   = qskt(QQ2,lc,sc,sh,df);
        QST_store(jt,:) = QST;
        STpar = [lc sc sh df];
        STpar_store(jt,:) = STpar;
        ...
        Temp =  PST(jt,:) .* (YY < QST(jq50));
        LeftEntropyST(jt,:) = -sum((log(PSTunc) - log(PST(jt,:))) .* Temp * delta);
        
        Temp =  PST(jt,:) .* (YY > QST(jq50));
        RightEntropyST(jt,:) = -sum((log(PSTunc) - log(PST(jt,:))) .* Temp * delta);
        
        qqTemp = qskt([delta1:delta1:alpha], STpar(1), STpar(2), STpar(3), STpar(4));
        EsfST(jt,:) = 1/alpha * sum(qqTemp * delta1);
        
        qqTemp = qskt([1-delta1:-delta1:1-alpha], STpar(1), STpar(2), STpar(3), STpar(4));
