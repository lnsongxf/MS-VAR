function [lc,sc,sh,df] = QuantilesInterpolation(qqTarg,QQ,Qmatch,lc0,sc0,sh0)
...
[~, jq25] = min(abs(QQ - Qmatch(2)));
[~, jq75] = min(abs(QQ - Qmatch(3)));
[~, jq05] = min(abs(QQ - Qmatch(1)));
[~, jq95] = min(abs(QQ - Qmatch(4)));

%% Set initial conditions for optimization (if not provided)
if nargin < 4
    iqn = norminv(Qmatch(3)) - norminv(Qmatch(2));    
    lc0 = qqTarg(jq50);
