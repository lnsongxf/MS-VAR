...
foreign = 1; % =1 if foreign test 

% Model label
if foreign ==0
    model_label = ['_const_' num2str(const) '_normal_20201108_' num2str(normal)];
elseif foreign ==1
    model_label = ['_const_' num2str(const) '_normal_20201108_foreign_' num2str(normal)];
end
...
model = 'SVAR_StateDependent_2Regimes_foreign';
...
elseif foreign ==1
    load data_full_20201108_foreign
end
...
if exist('ResMatch')==0
    load('results___Country=Foreign_TW___GDP=GDP___SpecWith=ff_mf___Scenario=November11_Smooth___Lags=0___Detrended=given___Sample=1973-Jan_to_2020-Oct.mat')
end
...
%             if normal
%                 % Transition probabilities when coefficients have normal prior
%                 p12 = 1./(1+exp(1-b12*(FF_COVID)-c12*(MF_COVID)));
%                 p21 = 1./(1+exp(1-b21*(FF_COVID)-c21*(MF_COVID)));
%             else
%                 % Transition probabilities when coefficients have gamma prior
%                 p12 = 1./(1+exp(1-b12.*(FF_COVID)+c12.*(MF_COVID)));
%                 p21 = 1./(1+exp(1+b21.*(FF_COVID)-c21.*(MF_COVID)));
...
for i=1:options.N
    ...
    for nsim=1:nDraws
        ...
        elseif const==0
            if normal
                % Transition probabilities when coefficients have normal prior
                p12 = 1./(1+exp(-b12*(FF_COVID)-c12*(MF_COVID)));
                p21 = 1./(1+exp(-b21*(FF_COVID)-c21*(MF_COVID)));
            else
                % Transition probabilities when coefficients have gamma prior
                p12 = 1./(1+exp(-b12.*(FF_COVID)+c12.*(MF_COVID)));
                p21 = 1./(1+exp(+b21.*(FF_COVID)-c21.*(MF_COVID)));
...
%% Index for April 2019

% Index for September 2019
t_sep_2019 = find(datenum('2019-Apr',dataformat)==dates);
...
% MF_COVID = DATA_APR2.MF_US(556:567);
% TR_COVID = DATA_APR2.TREND_US(556:567);
FF_COVID = db_full.FF.data(end-11-6:end-6);
MF_COVID = db_full.MF.data(end-11-6:end-6);
TR_COVID = db_full.TRENDH.data(end-11-6:end-6);
...
% Start waitbar
updateWaitbar = waitbarParfor(nParamDraws, "Covid-19 distribution: Simulating for April (Nov 8)...");
...
for i=1:options.N
    ...
    for nsim=1:nDraws
        ...
        % **** Draw the initial state in April 2019
        
        % regime-2 ***
        st_lag = prob_reg2_new(t_march_2019);
        
        % Get transition probabilities from May-2019 to April-2020
        if const==1
            if normal
                % Transition probabilities when coefficients have normal prior
                p12 = 1./(1+exp(a12-b12*(FF_COVID)-c12*(MF_COVID)));
        ...
        % Draw Markov Chain from May-2019 to April-2020
        for tt2 = 1:12
...
% Average out simulations -> returns a TxmParamDraws matrix
y_erg_barcovid_nov8_apr = (reshape(dY_erg_matcovid,1,nDraws*nParamDraws));
...
for i=1:options.N
    ...
    for nsim=1:nDraws
        ...
        % Get transition probabilities from April-2019 to March-2020
        if const==1
            if normal
                % Transition probabilities when coefficients have normal prior
                p12 = 1./(1+exp(a12-b12*(FF_COVID)-c12*(MF_COVID)));
...
[pdf_apr,xi_apr]=ksdensity(y_erg_barcovid_nov8_apr(1,:));
[cdf_apr,xic_apr]=ksdensity(y_erg_barcovid_nov8_apr(1,:),'function','cdf');
...
hleg = legend([l1 l3 l4],'Orientation','Vertical','Location','NW','interpreter','Latex');legend boxoff;
...
if saveit==1
    print('-dpdf',fig,[fig_folder 'Mar_Oct_2020_foreign'],'-bestfit');
    %saveas(fig,sprintf('%s.png',[fig_folder 'DensityQR']));
end
...
tind22 = find(datenum(char(period_pdf22),dataformat)==dates_full)+1;
...
[~,i25_ms_aug] = min(abs(cdf_aug-0.25));
[~,i25_ms_apr] = min(abs(cdf_apr-0.25));
[~,i25_ms_mar] = min(abs(cdf_mar-0.25));
[~,imedian_ms_oct] = min(abs(cdf_oct-0.5));

ym(t) = max(PST(tind2,:)');
ymax = max(ym)*1.05;
ymax = max([max(pdf_aug)*1.05]);
...
plot(ax1,xi_sep,pdf_sep,'LineWidth', 2,'Color',colors2(6,:),'DisplayName',['2020-Sep']);
%  plot(ax1,xi_aug,pdf_aug,'LineWidth', 2,'Color',colors2(6,:),'DisplayName',['2020-Aug']);
% plot(ax1,xi_apr,pdf_apr,'LineWidth', 2,'Color',colors2(16,:),'DisplayName',['2020-Apr']);
...
% plot(ax1,xi_apr(i25_ms_apr),pdf_apr(i25_ms_apr),'d','MarkerFaceColor',colors2(16,:),'MarkerEdgeColor','black','Markersize',10,'DisplayName',['25th Quantile = ' num2str(round(xi_apr(i25_ms_apr),1)) '\% (April)']);
...
tightfig;
if saveit==1
    print('-dpdf',fig,[fig_folder 'DensityMS_Mar_Sep_Oct_foreign'],'-bestfit');
...
[~,i10_sep] = min(abs(CDF(tind22,:)'-0.1));
[~,i25_sep] = min(abs(CDF(tind22,:)'-0.25));
[~,imedian_sep] = min(abs(CDF(tind22,:)'-0.5));
[~,i10_aug] = min(abs(CDF(tind2,:)'-0.1));
[~,i25_aug] = min(abs(CDF(tind2,:)'-0.25));
[~,imedian_aug] = min(abs(CDF(tind2,:)'-0.5));
...
% l2=plot(ax1,ResMatch.YY,PST(tind22,:)','-.','Color',colors2(6,:),'LineWidth',2,'DisplayName',['2020-Sep (QR)']);
% l22=plot(ax1,xi_sep,pdf_sep,'-','LineWidth', 2,'Color',colors2(6,:),'DisplayName',['2020-Sep (MS-VAR)']);
...
% l7=plot(ax1,ResMatch.YY(:,i25_sep),PST(tind22,i25_sep)','d','MarkerFaceColor',colors2(6,:),'MarkerEdgeColor','black','Markersize',10);
% plot(ax1,xi_sep(i25_ms_sep),pdf_sep(i25_ms_sep),'s','MarkerFaceColor',colors2(6,:),'MarkerEdgeColor','black','Markersize',10);
...
tightfig;
if saveit==1
%     print('-dpdf',fig,[fig_folder 'DensityQR_Mar_Sep_Oct_foreign'],'-bestfit');
%     print('-dpdf',fig,[fig_folder 'DensityQR_Mar_Aug_Oct_foreign'],'-bestfit');
    print('-dpdf',fig,[fig_folder 'DensityQR_Mar_Oct_foreign'],'-bestfit');
    %saveas(fig,sprintf('%s.png',[fig_folder 'DensityQR']));
end
...
MF_COVID_short = DATA_SYN.MF_FN(end-11-2:end-2);
FF_COVID_add = DATA_SYN.FF(end-1:end);
MF_COVID_add = DATA_SYN.MF_FN(end-1:end);
% TR_COVID = DATA_APR2.TREND_US(556:567);
% FF_COVID = db_full.FF.data(end-11:end);
% MF_COVID = db_full.MF.data(end-11:end);
TR_COVID = db_full.TRENDH.data(end-11:end);
...
[pdf_dec_syn,xi_dec_syn]=ksdensity(y_erg_barcovid_dec_syn(1,:));
[cdf_dec_syn,xic_dec_syn]=ksdensity(y_erg_barcovid_dec_syn(1,:),'function','cdf');
...
tightfig;
if saveit==1
    print('-dpdf',fig,[fig_folder 'DensityMS_Dec_Syn_foreign'],'-bestfit');
%     print('-dpdf',fig,[fig_folder 'DensityMS_Mar_Aug_Oct_foreign'],'-bestfit');
%     print('-dpdf',fig,[fig_folder 'DensityMS_Mar_Oct_foreign'],'-bestfit');
    %saveas(fig,sprintf('%s.png',[fig_folder 'DensityQR']));
end
...
tightfig;
if saveit==1
    print('-dpdf',fig,[fig_folder 'DensityMS_Dec_Regs_foreign'],'-bestfit');
%     print('-dpdf',fig,[fig_folder 'DensityMS_Mar_Aug_Oct_foreign'],'-bestfit');
%     print('-dpdf',fig,[fig_folder 'DensityMS_Mar_Oct_foreign'],'-bestfit');
    %saveas(fig,sprintf('%s.png',[fig_folder 'DensityQR']));
end
