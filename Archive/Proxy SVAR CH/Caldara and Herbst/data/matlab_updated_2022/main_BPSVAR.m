...
% DANILO'S NOTE: Fixed the degrees of freedom (line 139) from nnuT = T - n*p -nex_;
% to nnuT = T - nnu0;
...
% SETTINGS
%------------------------------------------------------------
model_vec = {'FF_MF_GDP'};                    % Select Model ('4eq', '5eq', 'FF_MF', or 'FF_MF_GDP')
instr_choice = 4;                       % 1 --> Original paper
                                        % 2 --> MP shock Miranda-Agrippino and Ricco (updated through Dec/2015)
                                        % 3 --> Info shock Miranda-Agrippino and Ricco (updated through Dec/2015)
                                        % 4 --> MP shock (updated through Jan/2022)
                                        
shadow = 1;                             % 1 --> Wu-Xia Shadow rate
p = 12;                                 % Number of lags
nex_ = 1;                               % Constant
...
elseif instr_choice ==2 || instr_choice ==3
    if instr_choice ==2; instrList = {'MPI_FF4'}; elseif instr_choice ==3; instrList = {'INFO_FF4'}; end
    i_ffr = 'EFFR';
    str_sample_init = '1990-01-01';         % Starting date of the sample (include pre-sample)
    str_sample_end  = '2015-12-01';         % End date of the sample
    str_iv_init     = '1994-01-01';         % Starting date of the sample for the proxy
elseif instr_choice ==4
    instrList = {'MPS'};
    i_ffr = 'EFFR';
    str_sample_init = '1990-01-01';         % Starting date of the sample (include pre-sample)
    str_sample_end  = '2019-12-01';         % End date of the sample
    str_iv_init     = '1994-01-01';         % Starting date of the sample for the proxy
end
if shadow ==1; i_ffr = 'EFFR_SR'; end
...
for mCounter = 1:size(model_vec,2)
    ...
    if strcmp(mmodel, '5eq')
        ...
        i_var_str =  {i_ffr, 'LIPM', 'UNRATE', 'LPPI', 'BAA10YMOODY'};
    ...
    elseif strcmp(mmodel, '4eq')
        i_var_instr = instrList;
        i_var_str =  {i_ffr, 'LIPM', 'UNRATE', 'LPPI'};
        i_var_transf =  {};     % Variable that require additional transformations
        nCalc = length(i_var_transf);
        i_var_str_names =  i_var_str; % Name of variables (for plots)
        varSelec = [1 2 3 4]; % Select variables to plot
    elseif strcmp(mmodel, 'FF_MF')
        i_var_instr = instrList;
        i_var_str =  {i_ffr, 'MF', 'UNRATE', 'LPPI', 'FF'};
        i_var_transf =  {};     % Variable that require additional transformations
        nCalc = length(i_var_transf);
        i_var_str_names =  i_var_str; % Name of variables (for plots)
        varSelec = [1 2 3 4 5]; % Select variables to plot
    elseif strcmp(mmodel, 'FF_MF_GDP')
        i_var_instr = instrList;
        i_var_str =  {i_ffr, 'MF', 'UNRATE', 'LPPI', 'FF','MGDP'};
        i_var_transf =  {};     % Variable that require additional transformations
        nCalc = length(i_var_transf);
        i_var_str_names =  i_var_str; % Name of variables (for plots)
    ...
    data_file = '../data/CHdata_2022.txt';
    ...
    savefileirf = strcat('./results/Result_',char(mmodel),char(i_var_instr),char(i_ffr),'p_',num2str(p),'_pr_',prior_type,'MP_',num2str(MP),'.mat');
    save(savefileirf,'SVAR'); 
    toc

    vm_plot_irf_bvar_mod(mmodel,prior_type,i_var_instr,i_ffr,p,MP,fflagFEVD,printFig)
    diary off
end
