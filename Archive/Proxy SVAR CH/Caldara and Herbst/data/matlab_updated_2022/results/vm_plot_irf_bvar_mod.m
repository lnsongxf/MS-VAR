function vm_plot_irf_bvar_mod(mmodel,prior_type,i_var_instr,i_ffr,p,MP,fflagFEVD,printFig)

graph_opt.font_num = 10;
load(strcat('./results/Result_',char(mmodel),char(i_var_instr),char(i_ffr),'p_',num2str(p),'_pr_',prior_type,'MP_',num2str(MP),'.mat'))
...
for jj = 1:nshockplot % Shock
    ...
    if printFig
        print('-dpdf',strcat('./results/IRF_',char(mmodel),char(i_var_instr),char(i_ffr),'p_',num2str(p),'_priortype_',prior_type, '_MP_',num2str(MP),'.pdf'))

        print('-dpdf',strcat('./results/IRF_',char(mmodel),char(i_var_instr),char(i_ffr),'p_',num2str(p),'_prtrunc_',prior_type,'_MP_',num2str(MP),'.pdf'))
    end
%     Plot FEVD
    if fflagFEVD ==1
    nvW = length(varSelec);
...
box off
    ...
    if printFig
        print('-dpdf',strcat('./results/FEVD_',char(mmodel),char(i_var_instr),char(i_ffr),'p_',num2str(p),'_priortype_',prior_type, '_MP_',num2str(MP),'.pdf'))
    end
    end
end
