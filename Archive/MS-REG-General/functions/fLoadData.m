...
for jj=1:length(varlist)
    data_use.(varlist{jj}) = rawdata.(varlist{jj})(s:e_sim);
    data_use.([varlist{jj} '_TREND']) = rawdata.([varlist{jj} '_TREND'])(s:e_sim);
end
...
for jj=1:length(varlist)
    ...
    Yh_trend(1:hh-1) = NaN;
    yh_trend.(varlist{jj}) = [Yh_trend(hh:e_sim); zeros(hh-1,1)];
    ...
    if hh==0
        ...
        Yh(1:hh-1) = NaN;
        yh.(varlist{jj}) = [Yh(hh:e_sim); zeros(hh-1,1)];
        
        % Series with trend
        Yh_wt = filter(ones(1,hh)/hh, 1, data_use.(varlist{jj}));
        Yh_wt(1:hh-1) = NaN;
        yh_wt.(varlist{jj}) = [Yh_wt(hh:e_sim); zeros(hh-1,1)];
