...
sv0=svar(opt.varlist,exog,opt.nlags,constant,panel,markov_chains);
...
if opt.identify ==1
    ...
    for jj=1:(length(idx))
        ...
        for kk = ind:length(idx)
            a0_const = [a0_const ['a0(' num2str(jj) ',' opt.varlist{kk} ')=0']];
        end
    end
elseif opt.identify ==2
end
...
if opt.dir_or_it ==1
    for ilag=1:opt.nlags
        ...
        a1_const=[a1_const
            {
            ['a',mylag,'(' num2str(idxf) ',' opt.varlist{idxg} ')=0'] % No effect of GDP(t-i) on FF
            ['a',mylag,'(' num2str(idxm) ',' opt.varlist{idxg} ')=0'] % No effect of GDP(t-i) on MF
            }];
    end
end
if modelspec==10
    for ilag=1:opt.nlags
        ...
        a1_const=[a1_const
            {
            ['a',mylag,'(' num2str(idxg) ',' opt.varlist{idxf} ')=0'] % No effect of FF(t-i) on GDP
            ['a',mylag,'(' num2str(idxg) ',' opt.varlist{idxm} ')=0'] % No effect of MF(t-i) on GDP
            }];
    end
elseif modelspec==11 || modelspec==13
    for ilag=1:opt.nlags
        ...
        a1_const=[a1_const
            {
            ['a',mylag,'(' num2str(idxg) ',' char(opt.varlist{idxg}) ')=0'] % No effect of GDP(t-i) on GDP
            }];
    end
...
for istate=1:numberOfStates
    ...
    if opt.identify ==1
        ...
        for jj=1:(length(idx))
            ind = ind+1;
            for kk = ind:length(idx)
                a0_markov = [a0_markov; ['a0(' num2str(jj) ',' opt.varlist{kk} ',sync,',mystate,')=0']];
            end
        end 
    elseif opt.identify ==2
    end
    ...
    if opt.dir_or_it ==1
        for ilag=1:opt.nlags
            ...
            a1_markov=[a1_markov
                {
                ['a',mylag,'(' num2str(idxf) ',' opt.varlist{idxg} ',sync,',mystate,')=0']    % No effect of GDP(t-i) on FF
                ['a',mylag,'(' num2str(idxm) ',' opt.varlist{idxg} ',sync,',mystate,')=0']    % No effect of GDP(t-i) on MF
                }];
        end
    end
    if modelspec==12
        for ilag=1:opt.nlags
            ...
            a1_markov=[a1_markov
                {
                ['a',mylag,'(' num2str(idxg) ',' opt.varlist{idxg} ',sync,',mystate,')=0']    % No effect of GDP(t-i) on GDP
                }];
        end
    elseif modelspec==9  || modelspec>100 && modelspec<200
        for ilag=1:opt.nlags
            ...
            a1_markov=[a1_markov
                {
                ['a1(' num2str(idxg) ',' opt.varlist{idxg} ',sync,',mystate,')=0']  % No effect of GDP(t-1) on GDP
                }];
        end
    else
        a1_markov=[a1_markov
