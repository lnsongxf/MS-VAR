...
for imodel=mymodels
    ...
    dates_oos = datenum((datetime(efirstok,'InputFormat',inputformat))+calmonths(1):calmonths(1):(datetime(end_date_IS,'InputFormat',inputformat)))';
    
    % Indexes in OOS estimation sample
    sd_oos      = find(datenum((datetime(start_plot,'InputFormat',inputformat))+calmonths(1))==dates_oos);
    ...
    sd_full      = find(datenum((datetime(start_plot,'InputFormat',inputformat))+calmonths(1))==dates_full);
    ...
    for i=1:maxwin
        ...
        if exist(sim_file,'file') && opt.freshsim == 0
            ...
            elseif opt.fastsim
                ...
                results.quantiles.st_h_mean = quantiles.st_h_mean;
        ...
        benchmark.st_h_mean(i,1) = results.quantiles.st_h_mean;   % transition probability
    ...
    if opt.showfig==1
        ...
        if opt.selfig(1)
            ...
            l4=plot(dates_oos(sd_oos:ed_oos), 1-benchmark.st_h_mean(sd_oos:ed_oos),'-','Color','k','LineWidth', 2.5,'DisplayName','Simulated $s(t+1,t+h)$');
            l5=plot(dates_full(sd_full:ed_full), 1-IS_results.quantiles.st_t_mean(sd_full:ed_full),'-.','Color',colors(10,:),'LineWidth', 2.5,'DisplayName','Simulated $s(t)$ in-sample');
            l6=plot(dates_full(sd_full:ed_full), 1-IS_results.quantiles.st_h_mean(sd_full:ed_full),'-.','Color','k','LineWidth', 2.5,'DisplayName','Simulated $s(t+1,t+h)$ in-sample');
