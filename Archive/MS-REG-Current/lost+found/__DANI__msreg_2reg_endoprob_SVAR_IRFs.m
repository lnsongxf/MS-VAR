...
%%
    ...
    left_color= [0 0 0];
    right_color= colors(2,:);
    bands = [50,10,25,75,90];
%     bands = [50,2.5,16,84,97.5];
    for ii=opt.date_index
        irf_bands = NaN(3,opt.hh,length(bands));
        prob_bands = NaN(2,opt.hh);
        irf_temp1  = squeeze(quantiles.y_mat_IRF(:,ii,:));
        irf_temp2  = squeeze(quantiles.y_mat_IRF_good(:,ii,:));
        irf_temp3  = squeeze(quantiles.y_mat_IRF_bad(:,ii,:));
        prob_temp1 = squeeze(quantiles.stmat_hh_IRF(:,ii,:));
        prob_temp2 = squeeze(quantiles.stmat_hh_IRF_shock(:,ii,:));
        irf_bands(1,:,:) = prctile(irf_temp1,bands)';
        irf_bands(2,:,:) = prctile(irf_temp2,bands)';
        irf_bands(3,:,:) = prctile(irf_temp3,bands)';
        prob_irf = mean(prob_temp2,1) - mean(prob_temp1,1);
        
        ylim1 = min(min(min(irf_bands)));ylim2 = max(max(max(irf_bands)));
        xaxis = 1:opt.hh;
        figIRF = figure;
        for yy=1:4
            if yy<4
                irf_temp  = squeeze(irf_bands(yy,:,:));
            else
                irf_temp  = prob_irf'*100;
            end
            subplot(2,2,yy)
            plot(xaxis, zeros(opt.hh), '--k','color',[0.5 0.5 0.5]); hold on
            plot(xaxis, irf_temp(:,1), 'k-', 'LineWidth', 3);
            if yy <4; plot(xaxis, irf_temp(:,2:end), 'r-.', 'LineWidth', 2); end
            set(gca, 'FontSize', FontSize)
            ylabel('percent', 'FontSize', FontSize)
            %     xlabel('months', 'FontSize', font_size)
            set(gca, 'XTick', 1:1:opt.hh)
            axis tight
            if yy<4
                ylim([ylim1 ylim2]);
%             else
%                 ylim([0 1]);
            ...
            elseif yy ==2
             ...
             elseif yy ==3
                title('Bad regime') 
            elseif yy ==4
                title('Prob. good regime')              
            end
            hold off;
        end
        set(figIRF,'PaperOrientation','portrait');
        set(figIRF, 'PaperSize', figSize);
        set(figIRF, 'PaperUnits', 'inches');
        set(figIRF, 'Units','inches');
        set(figIRF, 'PaperPositionMode', 'auto');
        set(figIRF, 'Position', [figSize(1)/5 figSize(2)/5 figSize(1) figSize(2)]);
        tightfig;
    end
    ...
    for ii=1:numel(opt.date_index)
        ...
        [pdfi_shock(ii,:),xi_shock(ii,:)]  = ksdensity(pdensity.realized_shock(:,ii));
        [pdfi_good_shock(ii,:),xi_good_shock(ii,:)]  = ksdensity(pdensity.good_shock(:,ii));
        [pdfi_bad_shock(ii,:),xi_bad_shock(ii,:)]  = ksdensity(pdensity.bad_shock(:,ii));
        [cxi_shock(ii,:),cdfi_shock(ii,:)] = ksdensity(pdensity.realized_shock,'Function','icdf');
   end
...
%         prob = trapz(xi_full(ii,:),pdfi_full(ii,:))
    ...
    if opt.showfig==1
        ...
        %     ylim([-10 20])
            ...
            subplot(212)
            hold on
            l3=plot(dates_full(sd:ed_full), 1-quantiles.st_t_mean(sd:ed_full),'-','Color',colors(10,:),'LineWidth', 2.5,'DisplayName','Simulated $s(t)$');
            l4=plot(dates_full(sd:ed_varp), p_reg2_filtered(sd:ed_varp),'--','Color',colors(20,:),'LineWidth', 2.5,'DisplayName','Filtered $s(t|t-1)$');
            l5=plot(dates_full(sd:ed_varp), p_reg2_updated(sd:ed_varp),'-.','Color',colors(30,:),'LineWidth', 2.5,'DisplayName','Updated $s(t|t)$');
            l6=plot(dates_full(sd:ed_varp), p_reg2_smoothed(sd:ed_varp),':','Color',colors(45,:),'LineWidth', 2.5,'DisplayName','Smoothed $s(t|T)$');
    ...
    if opt.showfig==1
        if opt.selfig(6)
            fig6=figure;
            figSize6 = [12 6];
            plot(dates_full(sd:ed_varp),ps(sd:ed_varp),'Color',[0.3 0.3 0.3],'LineWidth', 2.5); 
            if opt.dir_or_it==1
...
%         psr = ps((recs(ii)-5):(rece(ii)+5));
    ...
    if opt.showfig==1
        ...
        %         text(xlimits(1)*0.865,0.8*ylimits(2),['Prob bad regime = ' num2str((1-quantiles.st_t_mean(opt.date_index(ii)))*100) '\%'],'FontSize',13,'Interpreter','Latex');
                plot(zeros(1,length(xi_bad(1,:))),zeros(1,length(pdfi_bad(1,:))),'*','DisplayName',['Prob bad regime = ' num2str(round((1-quantiles.st_h_mean(opt.date_index(ii)))*100,1)) '\%']);
                vline(0,'k--');
...
%%   SHOCKED DENSITIES 
    if opt.showfig==1
        if opt.selfig(2)
            ...
            for ii=1:numel(opt.date_index)
                ...
                fig22=figure;
                hold on
                plot(xi_shock(ii,:),pdfi_shock(ii,:),linetype{2},'Color',[0 0 0 ],'LineWidth', 3,'DisplayName',[datestr(dates_full(opt.date_index(ii)),dataformat)]); 
                plot(xi_good_shock(ii,:),pdfi_good_shock(ii,:),linetype{4},'Color',colors(60,:),'LineWidth', 3,'DisplayName','Good regime'); 
                plot(xi_bad_shock(ii,:),pdfi_bad_shock(ii,:),linetype{5},'Color',colors(1,:),'LineWidth', 3,'DisplayName','Bad regime'); 
                if opt.dir_or_it==1
                    title(['Direct, shocked: ' figureTitleTag ],'Interpreter','Latex');
                else
                    title(['Iterated, shocked: ' figureTitleTag ],'Interpreter','Latex');
                end
                axis tight
                ylimits = ylim; xlimits = xlim;
        %         text(xlimits(1)*0.865,0.8*ylimits(2),['Prob bad regime = ' num2str((1-quantiles.st_t_mean(opt.date_index(ii)))*100) '\%'],'FontSize',13,'Interpreter','Latex');
                plot(zeros(1,length(xi_bad_shock(1,:))),zeros(1,length(pdfi_bad_shock(1,:))),'*','DisplayName',['Prob bad regime = ' num2str(round((1-quantiles.st_h_mean_shock(opt.date_index(ii)))*100,1)) '\%']);
                vline(0,'k--');
        %         text(GDPGH_full(opt.date_index(ii)),0.95*ylimits(2),'realized value','color','red','FontSize',9);
                vline(GDPGH_full_wt(opt.date_index(ii)),'r:','Realized Value');
                legend('Orientation','Vertical','Location','NW','interpreter','Latex');legend boxoff;
    ...
    if opt.showfig==1
        if opt.selfig(3)
            ...
            %yyaxis left
            hold on
            plot(dates, fitgood(sd:ed_varp),'Color',colors(1,:),'LineWidth', 1.5)
            plot(dates, fitbad(sd:ed_varp),'--','Color',colors(15,:),'LineWidth', 1.5)
            hold off
            ylabel('Percent','interpreter','Latex','fontsize',10)
            leg = {'Good Regime','Bad Regime'};
            axis tight
            ax=gca;
            ax.XTick = datenum(dates(1:48:end));
            datetick('x','yyyy','keepticks')
    ...
    if opt.showfig==1
        if opt.selfig(4)
            fig4=figure;
            hold on
            plot(dates,y_fit(sd:ed_varp),'linewidth',2)
            plot(dates,realized(sd:ed_varp),'k','linewidth',2)
            hold off
            ylabel('Percent','interpreter','Latex','fontsize',10)
    ...
    if opt.showfig==1
        if opt.selfig(5)
            fig5=figure;
            hold on
            plot(dates,p_reg2_smoothed(sd:ed_varp),'linewidth',2)
            plot(dates,p_reg2_filtered(sd:ed_varp),'k','linewidth',2)
            hold off
    ...
    if opt.showfig && opt.saveit
        ...
        if opt.selfig(2)
            ...
            print('-dpdf',fig22,[figfolder 'Densities_shocked_' shock_tag '_' datestr(dates_full(opt.date_index(ii)),dataformat) '_' modelname ],'-bestfit');
            print('-dpdf',figIRF,[figfolder 'IRF_' shock_tag '_' datestr(dates_full(opt.date_index(ii)),dataformat) '_' modelname ],'-bestfit');
        end
        if opt.selfig(3)
