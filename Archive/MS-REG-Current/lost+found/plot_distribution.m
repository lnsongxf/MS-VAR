function ax = plot_distribution(varargin)
...
[X,Y,color_value,alpha_value,line_width,line_style] = parseinputs(varargin{:});
...
for i=1:numel(X)-1
    ...
    fill(Px,Py,color_value,'FaceAlpha',alpha_value,'EdgeColor','none','HandleVisibility','off');
end

ax = plot(X,MU,'LineStyle',line_style,'LineWidth',line_width,'Color',color_value);
hold off
...
function [X,Y,color_value,alpha_value,line_width,line_style] = parseinputs(varargin)
...
if nargin > minargs
    for i=(minargs+1):2:nargin
        ...
        PNAME = validatestring(PNAME,{'Alpha','Color','LineWidth','Prctile','LineStyle'},...
