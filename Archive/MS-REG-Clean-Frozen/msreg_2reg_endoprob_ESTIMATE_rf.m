...
% USER INPUT
%========================================
vartype      = 2;     % 0 = SVAR (Cholesky + A1), 1 = SVAR (Cholesky), 2 = RFVAR
dir_or_it    = 2;     % 1 = direct, 2 = iterated
hh           = 3;     % forecast horizon
...
modelname = [datafilename '_' sheetuse '_' startdb enddb ...
    '_' 'C' num2str(const) 'N' num2str(normal) 'H' num2str(hh) tag];
...
%     if exist([paramsfolder modelname '.mat'],'file')==0
        ...
        elseif vartype==1
            run scriptEstimateMode_unr.m;
        elseif vartype==2
            run scriptEstimateMode_rf.m;
        end
