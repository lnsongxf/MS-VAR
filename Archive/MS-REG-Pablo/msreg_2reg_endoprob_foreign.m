...
% Foreign recession dates
fnrecdates(1,:) = [datenum(datetime('1974-Sep','InputFormat',inputformat)) datenum(datetime('1974-Dec','InputFormat',inputformat))];
fnrecdates(2,:) = [datenum(datetime('1982-Mar','InputFormat',inputformat)) datenum(datetime('1982-Nov','InputFormat',inputformat))];
fnrecdates(3,:) = [datenum(datetime('1992-Feb','InputFormat',inputformat)) datenum(datetime('1992-Mar','InputFormat',inputformat))];
fnrecdates(4,:) = [datenum(datetime('2001-Apr','InputFormat',inputformat)) datenum(datetime('2001-Dec','InputFormat',inputformat))];
fnrecdates(5,:) = [datenum(datetime('2002-Dec','InputFormat',inputformat)) datenum(datetime('2003-Apr','InputFormat',inputformat))];
fnrecdates(6,:) = [datenum(datetime('2008-Apr','InputFormat',inputformat)) datenum(datetime('2009-May','InputFormat',inputformat))];
...
fig_folder = ['Results/Figures/' model '/' model_label '/'];
...
% Figure: Fitted and Realized GDP
fig=figure;
hold on
...
legend boxoff
axis tight
setfig(fig,dates,sd,ed,numticks,FontSize,figSize,'Fitted and Realized GDP');
if saveit==1
...
ylabel('Percent','interpreter','Latex','fontsize',10)
axis tight; ylim([-6.5 6.5])
setfig(fig,dates,sd,ed,numticks,FontSize,figSize,'Realized Innovation to GDP Equation');
...
rr = recessionplot('recessions',fnrecdates);
ylabel('Probability','interpreter','Latex','fontsize',10)
setfig(fig,dates,sd,ed,numticks,FontSize,figSize,'Smoothed Regime Probabilies and Realized GDP');
if saveit==1
...
legend boxoff
%rr = recessionplot('recessions',fnrecdates); 
setfig(fig,dates,sd,ed,numticks,FontSize,figSize,'Smoothed Regime Probabilies and Factors');
if saveit==1
...
%%
datatest = dY_sim_full;
%%

fig=figure;hold on;
[lineh, bandsh] = fanChart(dates_full(sd2:ed_full), datatest(sd2:ed_full,1:10:20000),'mean', [10 25 75 90], ...
    'alpha', .5, 'colormap', {'shadesOfColor', [0 0 .8]});
%txt = strcat({'Pct'}, cellstr(int2str()));
legend('a','b','c','d')
setfig(fig,dates_full,sd2,ed_full,numticks,FontSize,figSize,'Historical Quantiles 2007-2020');
...
%% Data structure with Nov-11 vintage 
data_nov11 = data_use;

s2 = find(datenum('2019-Oct',dataformat)==dates_full);
e2 = find(datenum('2020-Sep',dataformat)==dates_full);
data_nov11.MF_full(s2:e2)=[
-0.56
-0.48
-0.38
-0.01
...
% l3=plot(xi_dec_2,pdf_dec_2,'g--','LineWidth', 4,'DisplayName',['October-2020  (Bad Regime)']);
...
%% Figure: March, April, August, October, December 2020 
%[pdf_dec,xi_dec]=ksdensity(outDec2020.y_erg_bar(1,:));
...
num2str(round(xi_oct(i25_ms_oct),2)) 
...
%l7=plot([swave swave],[0 ylimits(2)],'r','LineWidth',1,'DisplayName',['Second Waves = ' num2str(round(swave,1)) '\%']);
hleg = legend([l1 l4],'Orientation','Vertical','Location','NW','interpreter','Latex');legend boxoff;
title('MS-VAR: Densitites for GDP growth over the next 12 months (Foreign)','FontSize',16','Interpreter','Latex');
ylabel('PDF','fontsize',10,'interpreter','Latex')
xlabel('1-Year-Ahead GDP Growth','fontsize',10,'Interpreter','Latex')
set(gca, 'FontName', 'Times New Roman');
set(gca, 'FontSize', FontSize);
...
%%
if saveit==1
    print('-dpdf',fig,[fig_folder 'Mar_Sep_Oct_2020_foreign'],'-bestfit');
    %saveas(fig,sprintf('%s.png',[fig_folder 'DensityQR']));
end
%%
%save data_plot_foreign_iedo_1130.mat xi_mar pdf_mar xi_sep pdf_sep xi_oct pdf_oct

save data_plot_foreign_bb_1130.mat xi_mar pdf_mar xi_apr pdf_apr xi_oct pdf_oct xi_oct_1 pdf_oct_1 xi_oct_2 pdf_oct_2 xi_mar_1 pdf_mar_1 xi_mar_2 pdf_mar_2
%%
return
load data_plot_foreign_bb_1130.mat
...
%% SIMULATE MODEL WITHOUT CONSTANT
return;

load data_foreign_no_constant.mat

outMar2020nc = fGetDensityMS('2020-Mar',data_use_nc,a2tilde_to_a_nc,results_nc,pnames_nc,nDraws_nc,nParamDraws_nc);
outApr2020nc = fGetDensityMS('2020-Apr',data_use_nc,a2tilde_to_a_nc,results_nc,pnames_nc,nDraws_nc,nParamDraws_nc);
outSep2020nc = fGetDensityMS('2020-Sep',data_use_nc,a2tilde_to_a_nc,results_nc,pnames_nc,nDraws_nc,nParamDraws_nc);
outOct2020nc = fGetDensityMS('2020-Oct',data_use_nc,a2tilde_to_a_nc,results_nc,pnames_nc,nDraws_nc,nParamDraws_nc);

[pdf_mar_nc,xi_mar_nc]=ksdensity(outMar2020nc.y_erg_bar(1,:));
[pdf_apr_nc,xi_apr_nc]=ksdensity(outApr2020nc.y_erg_bar(1,:));
[pdf_sep_nc,xi_sep_nc]=ksdensity(outSep2020nc.y_erg_bar(1,:));
[pdf_oct_nc,xi_oct_nc]=ksdensity(outOct2020nc.y_erg_bar(1,:));


%%
fig=figure;
hold on
l1=plot(xi_apr,pdf_apr,'Color',colors(1,:),'LineWidth', 3,'DisplayName',['April-2020 ']); hold on;
l2=plot(xi_sep,pdf_sep,'Color',colors(22,:),'LineWidth', 3,'DisplayName',['September-2020 ']);
l3=plot(xi_oct,pdf_oct,'Color',colors(60,:),'LineWidth', 3,'DisplayName',['October-2020 ']);
l4=plot(xi_apr_nc,pdf_apr_nc,'--','Color',colors(1,:),'LineWidth', 3,'DisplayName',['April-2020 (no constant) ']); hold on;
l5=plot(xi_sep_nc,pdf_sep_nc,'--','Color',colors(22,:),'LineWidth', 3,'DisplayName',['September-2020 (no constant) ']);
l6=plot(xi_oct_nc,pdf_oct_nc,'--','Color',colors(60,:),'LineWidth', 3,'DisplayName',['October-2020 (no constant) ']);

ylimits = ylim; xlimtis = xlim;
plot(zeros(10,1),linspace(0,ylimits(2),10),'k--');
%l7=plot([staff staff],[0 ylimits(2)],'b','LineWidth',1,'DisplayName',['Staff forecast = ' num2str(round(staff,1)) '\%']);
%l8=plot([swave swave],[0 ylimits(2)],'r','LineWidth',1,'DisplayName',['Second Waves = ' num2str(round(swave,1)) '\%']);
hleg = legend([l1 l2 l3 l4 l5 l6 l7 l8],'Orientation','Vertical','Location','NW','interpreter','Latex');legend boxoff;
title('MS-VAR: Densitites for GDP growth over the next 12 months (Foreign)','FontSize',16','Interpreter','Latex');
ylabel('PDF','fontsize',10,'interpreter','Latex')
xlabel('1-Year-Ahead GDP Growth','fontsize',10,'Interpreter','Latex')
set(gca, 'FontName', 'Times New Roman');
...
%print('-dpdf',fig,[fig_folder 'Mar_Sep_Oct_2020_foreign_const_noconst'],'-bestfit');
