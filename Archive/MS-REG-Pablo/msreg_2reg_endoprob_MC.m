...
for nsim=1:nDraws
    ...
    % Simulate DGP states through period 11
    p12_dgp = 1/(1+exp(dgp.a12-dgp.b12*(FF(1:11))+dgp.c12*(MF(1:11))));
    p21_dgp = 1/(1+exp(dgp.a21+dgp.b21*(FF(1:11))-dgp.c21*(MF(1:11))));
    
    p11_dgp = ones(11,1) - p12_dgp; % probability of remaining in normal
    p22_dgp = ones(11,1) - p21_dgp; % probability of remaining in bad
    
    % State in period 0
    st_lag_dgp  = st_lag;
       
       % Draw regimes from 1:11
       for tt2 = 1:11
           
           % Draw the Markov Chain for period t-12:t
           udraw = rand(1);
           
           if st_lag_dgp == 0 % started in good regime
               if udraw > p11_dgp(tt2)
                   st_mat_dgp(tt2,1) = 1; % switch from good to bad
               else
                   st_mat_dgp(tt2,1) = 0; % don't switch and remain in good
               end
               
           else % start in bad regime
               
               if udraw > p22_dgp(tt2)
                   st_mat_dgp(tt2,1) = 0; % switch from bad to good
               else
                   st_mat_dgp(tt2,1) = 1; % don't switch and remain in bad
               end
               
           end           
           
           st_lag_dgp = st_mat_dgp(tt2,1);
       end
    ...
    for tt= 12:Tobs
        ...
        for tt2 = 1:12
            ...
            if st_lag == 0 % started in good regime
                if udraw > p11(tt2)
                    st_sim(tt2,1) = 1; % switch from good to bad
                else
                    st_sim(tt2,1) = 0; % don't switch and remain in good
                end
                
            else % start in bad regime
                
                if udraw > p22(tt2)
                    st_sim(tt2,1) = 0; % switch from bad to good
                else
                    st_sim(tt2,1) = 1; % don't switch and remain in bad
                end
        ...
        if st==0
            ...
            dY_sim(tt,nsim) = (pmode.c_3_1_sync_1 - pmode.a0_3_1_sync_1*FF(tt)...
                - pmode.a0_3_2_sync_1*MF(tt) + pmode.s_3_3_sync_1*shocks_sim(tt,nsim));
        else
            dY_sim(tt,nsim) = (pmode.c_3_1_sync_2 - pmode.a0_3_1_sync_2*FF(tt)...
        ...
        for jj=1:13
            ...
            if jj==1
                ...
                eta1 = randn(2,1); % financial and macro shocks                
                m_temp = dgp.c_2_1 +                          dgp.a1_2_1*f_lag + dgp.a1_2_2*m_lag + dgp.s_2_2*eta1(2,1);
                f_temp = dgp.c_1_1 + dgp.a0_1_2*m_temp*(-1) + dgp.a1_1_1*f_lag + dgp.a1_1_2*m_lag + dgp.s_1_1*eta1(1,1);                                
            end
            
            p12_dgp = 1/(1+exp(dgp.a12-dgp.b12*(f_use)+dgp.c12*(m_use)));
            p21_dgp = 1/(1+exp(dgp.a21+dgp.b21*(f_use)-dgp.c21*(m_use)));
            ...
            if st_temp==2
                DGP_temp(jj,1) = dgp.c_3_1_sync_2 - dgp.a0_3_1_sync_2*f_use - dgp.a0_3_2_sync_2*f_use + dgp.s_3_3_sync_2*eta2;
            else
                DGP_temp(jj,1) = dgp.c_3_1_sync_1 - dgp.a0_3_1_sync_1*f_use - dgp.a0_3_2_sync_1*m_use + dgp.s_3_3_sync_1*eta2;
            end
            
            % Update lagged factors
            f_lag = f_use;
            m_lag = m_use;
        ...
        DGP_sim(tt,nsim) = mean(DGP_temp(2:end));
...
dY_25_fut2 = prctile(DGP_sim',25)';
dY_75_fut2 = prctile(DGP_sim',75)';
dY_10_fut2 = prctile(DGP_sim',10)';
dY_90_fut2 = prctile(DGP_sim',90)';
...
l1=plot(dY_10_fut2(12:200),':','Color',colors(5,:),'LineWidth', 1.5,'DisplayName','DGP 10th');
l2=plot(dY_90_fut2(12:200),':','Color',colors(55,:),'LineWidth', 1.5,'DisplayName','DGP 90th');
l3=plot(dY_sim_10(12:200),'Color',colors(5,:),'LineWidth', 2,'DisplayName','MS-VAR 10th');
l4=plot(dY_sim_90(12:200),'Color',colors(55,:),'LineWidth', 2,'DisplayName','MS-VAR 90th');
l5=plot(y_mat_fut(draw,13:200),'k-.','LineWidth',2,'DisplayName','Data');
