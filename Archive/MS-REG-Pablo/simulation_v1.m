...
if Reg_variable==1
    Yraw = Yraw(1:end-12,:); % We only have filtered until Oct/2019
    GDPraw = GDPraw(1:end-12,:); % We only have filtered until Oct/2019
    MFraw = MFraw(1:end-12,:); % We only have filtered until Oct/2019
    FFraw = FFraw(1:end-12,:); % We only have filtered until Oct/2019
end
...
if Reg_variable ==1
    p_reg2_filtered = p_reg2_filtered(nlags+1:end);
    p_bad = false(size(p_reg2_filtered));
    p_bad(p_reg2_filtered>0.9) = true; % If 1, filtered bad regime
elseif Reg_variable ==2
    REC_US = REC_US(nlags+1:end,1);
    p_bad = false(size(REC_US));
    p_bad(REC_US==1) = true; % If 1, recession
elseif Reg_variable ==3
    reg_low = reg_low(nlags+1:end,1);
    p_bad = false(size(reg_low));
    p_bad(reg_low==1) = true; % If 1, recession
end    
...
st_draw = NaN(tt,1); p12_draw = NaN(tt-burn,1); p21_draw = NaN(tt-burn,1);
st_draw(1,1) = 1; % start in the normal regime
...
%     for jj=2:tt
%         eta1 = randn(2,1);
%         %         f_draw(jj) = b22*f_draw(jj-1)+eta1(1,1);
%         %         m_draw(jj) = b33*m_draw(jj-1)+eta1(2,1);
%         ind_f_m = round(rand(1)*(length(RESID_FF))+0.5,0); % draw a random combination of MF and FF historical innovations
%         f_draw(jj) = b22*f_draw(jj-1)+RESID_FF(ind_f_m);
%         m_draw(jj) = b33*m_draw(jj-1)+RESID_MF(ind_f_m);
%         %         p12 = 1/(1+exp(3-.1*(f_draw(jj))+.1*(m_draw(jj)))); % 'sync_tp_1_2=1/(1+exp(a12-b12*(FF)+c12*(MF)))'
%         %         p21 = 1/(1+exp(3+.1*(f_draw(jj))-.1*(m_draw(jj)))); % 'sync_tp_2_1=1/(1+exp(a21+b21*(FF)-c21*(MF)))'
%         p12 = 1/(1+exp(2.7-.23*(f_draw(jj))+.24*(m_draw(jj)))); % 'sync_tp_1_2=1/(1+exp(a12-b12*(FF)+c12*(MF)))'
%         p21 = 1/(1+exp(2.1+.19*(f_draw(jj))-.26*(m_draw(jj)))); % 'sync_tp_2_1=1/(1+exp(a21+b21*(FF)-c21*(MF)))'
%         
...
%         udraw = rand(1);
%         
%         if st_draw(jj-1) == 1 % started in good regime
%             if udraw > p11
%                 st_draw(jj,1) = 2; % switch from good to bad
%             else
%                 st_draw(jj,1) = 1; % don't switch and remain in good
%             end
%             
%         else % start in bad regime
%             
%             if udraw > p22
%                 st_draw(jj,1) = 1; % switch from bad to good
%             else
%                 st_draw(jj,1) = 2; % don't switch and remain in bad
%             end
%             
%         end
%         
%         eta2 = randn(1,1);
%         p12_draw(jj) = p12; p21_draw(jj) = p21;
%         
%         if st_draw(jj,1)==2
%             y_draw(jj) = mu_2+beta_2*f_draw(jj)+gamma_2*m_draw(jj)+sig_2*eta2; % I think we need to think about the innovation to GDP here
%         else
%             y_draw(jj) = mu_1+beta_1*f_draw(jj)+gamma_1*m_draw(jj)+sig_1*eta2;
%         end
%     end
%     y_mat(dd,:) = y_draw(burn+1:end); f_mat(dd,:) = f_draw(burn+1:end); m_mat(dd,:) = m_draw(burn+1:end); st_mat(dd,:) = st_draw(end);
%     
% end
...
for jj=2:tt
    ...
    %     f_draw(jj) = b22*f_draw(jj-1)+RESID_FF(ind_f_m);
    %     m_draw(jj) = b33*m_draw(jj-1)+RESID_MF(ind_f_m);
    p12 = 1/(1+exp(2-.1*(f_draw(jj))+.1*(m_draw(jj)))); % 'sync_tp_1_2=1/(1+exp(a12-b12*(FF)+c12*(MF)))' %USE THIS!
    p21 = 1/(1+exp(2+.1*(f_draw(jj))-.1*(m_draw(jj)))); % 'sync_tp_2_1=1/(1+exp(a21+b21*(FF)-c21*(MF)))' %USE THIS!
    %     p12 = 1/(1+exp(2.7-.23*(f_draw(jj))+.24*(m_draw(jj)))); % 'sync_tp_1_2=1/(1+exp(a12-b12*(FF)+c12*(MF)))'
    %     p21 = 1/(1+exp(2.1+.19*(f_draw(jj))-.26*(m_draw(jj)))); % 'sync_tp_2_1=1/(1+exp(a21+b21*(FF)-c21*(MF)))'
    ...
    for dd=1:rr
        ...
        % Below is the case where the GDP shock has the same sign of ind_f_m
        %         if RESID_S_total(ind_f_m)>0
        %             if eta2<0
        %                 eta2 = eta2*(-1);
        %             end
        %         elseif RESID_S_total(ind_f_m)<0
        %             if eta2>0
        %                 eta2 = eta2*(-1);
        %             end
        %         end
        ...
        if st_temp(dd)==2
            y_draw(jj) = mu_2+beta_2*f_draw(jj)+gamma_2*m_draw(jj)+sig_2*eta2;
        else
            y_draw(jj) = mu_1+beta_1*f_draw(jj)+gamma_1*m_draw(jj)+sig_1*eta2;
        end
        ...
        if jj>burn
            y_mat(dd,jj-burn) = y_draw(jj); st_mat(dd,jj-burn) = st_temp(dd);
        end
    end
    if jj>burn
...
hold on
% Plot percentiles from MS
l1=plot(1:tt-burn, mean(st_mat),'Color',colors(5,:),'LineWidth', 1.5,'DisplayName','States');
% l2=plot(1:tt-burn, ds_50,'Color',colors(5,:),'LineWidth', 1.5,'DisplayName','st 50th');
% l3=plot(1:tt-burn, ds_90,'Color',colors(5,:),'LineWidth', 1.5,'DisplayName','st 90th');
...
save Res_sim Res_iterated Res_direct
