...
%
%------------------------------------------------------------------
function [out,pden_mat] = fPredDensityDirectFull(sv,params_in,db_full,opt,temp)
...
for jj=1:nvars
    ...
    if jj == opt.idxg
        ...
        eval(['thetrend(:,jj) = db_full.' whichtrend opt.varlist{jj} '.data;']);
end
...
%     for uu=1:nDraws
        ...
        for tt=1:tperiods
            ...
            % Collect simulations
            y_mat_fut_ex_temp(dd,uu,:,tt) = y_temp' + thetrend(tt,:);
            y_mat_fut_ex_temp_good(dd,uu,:,tt) = y_temp1' + thetrend(tt,:);
            y_mat_fut_ex_temp_bad(dd,uu,:,tt)  = y_temp2' + thetrend(tt,:);
