...
% Last date for (in-sample) model evaluation with predictive score & PITs
...
% Number of simulations for each time period
opt.nDraws       = 5000;    

% Draws from posterior. has to be less than options.N;
opt.nParamDraws  = 1000;    
...
for ifor = myfor
    ...
    for ivar=1:size(opt.vars,1)
        ...
        for imodel=mymodels
            ...
            [db, db_full, datesorg, startdb, enddb,tex] = fLoadData(opt);            
            ...
            s=find(datesorg==opt.start_date);
            e=find(datesorg==opt.end_date);
            sp = find(datesorg==opt.start_plot);
            ep = find(datesorg==opt.end_plot);

            % Vector of dates for the full sample
            dates_full = datesorg(s:ep);
            
            % Starting date for plots
            sd      = find(opt.start_plot==dates_full);
            sd_varp = find(opt.start_plot==dates_full)+opt.nlags;
            
            % End date for plots
            ed_full  = find(opt.end_plot==dates_full);
            ed_varp  = find(opt.end_date==dates_full)-opt.hh;
            ed_edate = find(opt.end_date==dates_full);
            ed_eval  = find(opt.end_eval==dates_full);
            ...
            enddbf =  char(datetime(dates_full(ed_varp),'Format','yyyy''M''M'));
            ...
            opt.date_index = find(target_dates==dates_full);
            ...
            if opt.showfig==1
                if opt.selfig(1)
                    for nn=1:opt.nvars
                        ...
                        axis tight
                        set(gca,'XTick',dates_full(sd:numticks:ed_full))
                ...
                if opt.selfig(3)
                    ...
                    for nn=1:opt.nvars
                        for ii=1:numel(opt.date_index)
                            fig3 = figure;
                            hold on
                            plot(squeeze(results.xi(ii,nn,:)),squeeze(results.pdfi(ii,nn,:)),linetype{2},'Color',[0 0 0 ],'LineWidth', 3,'DisplayName',char(dates_full(opt.date_index(ii))));
...
%                             vline(realized(opt.date_index(ii)),'r:','Realized Value');
                            ...
                            tightfig;
                            if opt.saveit
                                print('-dpdf',fig3,[figfolder 'Densities_' opt.varlist '_' char(dates_full(opt.date_index(ii))) '_' modelname ],'-bestfit');
                            end
                        end
                    end
                end
            ...
            if opt.showfig==1
                if opt.selfig(4)
                    ...
                    axis tight
                    ax=gca;
                    ax.XTick = dates_full(sd:numticks:ed_varp);
                    datetick('x','yyyy','keepticks')
